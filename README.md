...
Как отредактирован мой /usr/bin/chromium-browser можно посмотреть открыв его так что я пока не понял зачем делают огромную баш портянку если я сделал так и все запускается задумаемся кто там работает.

Пока не понятно почему не работает вулкан хотя ранее удавалось включить его поддержку в более ранних версиях только они не могли совместно работать толи с opengl , skia , webgl или с чем то из них (точно не помню c каким компонентом по этому даю такую информацию может кто то быстрее найдет потерянный вулкан да как нибудь сорсы переправят что бы заработало все в купе) заити можно посмотреть по адресу chrome://gpu/ Vulkan: Disabled

_____________________________________________________________________________________________________________________________________________

Chrome 88 

New version google-chrome-unstable_88.0.4292.2-1 https://www.ubuntuupdates.org/pm/google-chrome-stable telemetry chrome://terms/ из за этого очень плохо работает мог бы собрать такой хромиум разгугленный с поддержкой ffmpeg , но нету денег на мощное оборудование и вопрос не в том что бы избежать телеметрии , а дело в том что этот браузер из за этого очень плохо работает с этими флагами , а всякие онлаин компиляторы и сеошнички лезут по компилировать свою библиотеку на тебе как на жертве по скольку у них на этот браузер есть счеты за счет удаленного управления проводить эксперименты что сразу видно с моими флагами. По этому я где то тут рекомендовал еще и удалить все ssl сертификаты они могут быть как доступ к управлению компьютером и если у вас не сайт они не нужны да придется отказаться от некоторых функции типа открыть во внешнем плеере mpv , totem видео с yotube или другого медиа сервера , но это лучше чем будут дампить и выдавать в виде нового дистрибутива ваш бэкэнд оси тем самым если вы как и я разрабатываете операционную систему , а точнее долатываете , исправляете и всячески пересобираете улучшая её скорость работы хотя есть не понимание и все же не то что бы предположение , а точное утверждение что автопроизвольного update-manager-core быть не должно и я думаю что мою операционную систему этим самым дискредитируют картельно сговорившись , но если честно признаются что у меня копилефтят бекграунд то тогда все в порядке будет особенно если придет обратная связь в виде каких то вознаграждении в эквиваленте того сколько я сделал за пять лет работы особенно за версию 20.04 , про ssl как пример https://xakep.ru/2018/03/08/ssl-tls-fuckup/ , для того что бы это было безопасно нужен какой никакой сандбокс песочница , а её нету и открывая видео таким образом не известно что происходит пример для mpv плеера mpv -v --no-check-certificate https://www.youtube.com/watch?v=9B-nTJyEZX0
...
griggorii@gmail.com нужно оборудование для разработки желательно конечно супер компьютер , но можно обойтись просто мощным железом желательно 16-32 разрядным процессором , на данный момент инвестиции в мою разработку нет , на уровне государства сидят те кто в основе делает вид что помогает. Мог бы перебрать хром если бы было все мощное и высоко скоростное с этими флагами , но нету оборудования так как надо его покупать , а это не в игрушки играть в ядре и тратить бюджет и многие кто собирает это понимает в ит сектор в россии подложена свинья в виде привилегированных ит отделов по этому даже мой дистрибутив не то что бы не поддержан инвестициями , но и (некторые на столько мерзкие сеошники что прямо выпрыгивают из трусов что бы отомстить за путина и прочии олигархат что банят кошельки объясняя это неким прошедшим временем по истечении которого данные кошелька исчезают , а такая платформа как ютуб окупированна странными людьми российского сегмента (навряд ли такую мерзость делают иностранцы у которых технологии ценятся на вес эипл макбука) которые могут говорить что это говно и забанить , а после выдать за новинку для своего человека из ит , кто эти все ратуют только за то что бы с помощью одного друга превращать деньги в нано деньги и никогда не помогали даже когда всем было пофиг чем они там щас в правительстве занимаются , кривая подвела к тому что пора все же этим заниматься даже вплотную и в один момент выкинуть их оттуда), но и не поддержан узнаваемостью что бы пользовались и все работало и дало как обход монетизации вне бюджета мог бы получить с той же платформы ютуба в которой указывался кошелек для обратной связи , но если кто заинтересован в альтернативном ядре и дистрибутиве то он тут https://youtu.be/9B-nTJyEZX0 естественно настройки подбираются что бы позднее дистрибутив работал ещё лучше , даже в данном случае настройки браузера в chrome://gpu/ могли бы уже быть пересобраны в хромиум по дефолту и навряд ли эти флаги не заработали на каких либо устройствах , пока дела плохи одни захватили станок , а другие инвестиции на деятельность и при том они уже как два года не могут выпустить полностью ускоренный браузер свои наработки я не беру с потолка за основу было взято исследование феномена первью от ютуба https://youtu.be/RrmsmJXYfTM позднее все настройки долго подобирались мной к этому результату https://youtu.be/I2aHs2mRtXc по сколько хромиум ос выдавал такое первью и был полностью ингуглед то навряд ли в медиа ключе была какая то уязвимость может только в авто воспроизведении ну так я могу и баш поставить на плей командой типа (make_eof_./my_bash.sh или make && ./my_bash.sh)<- вот и все автовоспроизведение не точно так же прибить можно будет такой скрипт два раза выполнив ctrl+c , а примерно , хотя в условиях современного веба об уязвимостях лучше вообще не рассуждать одна большая. Благо моими флагами можно сразу посмотреть результат работы не собирая как прототип пока я прибираюсь в настройках. На данный момент tls 1.3 для эксперимента отключен и все работает , но имеются проблемы с прокруткой колесом мыши все становиться очень медленным данный вопрос будет решен как будет найден тот флаг что мешает , а может это что то надо править в сорсах кода.
