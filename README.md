...
Пока не понятно почему не работает вулкан хотя ранее удавалось включить его поддержку в более ранних версиях только они не могли совместно работать толи с opengl , skia , webgl или с чем то из них (точно не помню c каким компонентом)  заити можно посмотреть по адресу chrome://gpu/ Vulkan: Disabled
